...
import com.example.ddd.configuration.config.Neo4jDriverFactory;
...
public class KgRelationRepository {
     ...
     */
    public void createDocToEntitiesRelation(String docId, String ragId, List<String> entityIds, 
                                           Map<String, List<String>> entitySentencesMap,
     ...
     */
    public void batchCreateRelations(Map<String, String> entityIdMap, 
                                     Map<String, String> entityTypeMap,
        ...
        neo4jDriverFactory.execute(tx -> {
            ...
            for (RelationDTO relation : relations) {
                String subjectId = entityIdMap.get(relation.getSubjectName());
                String objectId = entityIdMap.get(relation.getObjectName());
    ...
    public List<Map<String, Object>> queryEntitiesByDocId(String docId) {
        return neo4jDriverFactory.callableRead(tx -> {
            ...
            var entitiesList = new ArrayList<Map<String, Object>>();
            result.forEachRemaining(record -> {
                try {
                    ...
                    entitiesList.add(Map.of(
                        "entityId", record.get("entityId").asString(),
     ...
     * @return 实体ID到句子列表的映射
     */
    public Map<String, List<String>> querySentencesByDocIdAndEntityIds(String docId, List<String> entityIds) {
        if (entityIds == null || entityIds.isEmpty()) {
            return Map.of();
        }
        ...
        return neo4jDriverFactory.callableRead(tx -> {
            ...
            var sentencesMap = new java.util.HashMap<String, List<String>>();
            result.forEachRemaining(record -> {
                try {
     ...
     * @return 包含节点和边的图谱数据
     */
    public Map<String, Object> queryEntityGraphInDoc(String entityId, String docId) {
        return neo4jDriverFactory.callableRead(tx -> {
            // 先查询该文档中的所有实体ID
            ...
            var docEntityIds = new ArrayList<String>();
            docEntitiesResult.forEachRemaining(record -> {
                docEntityIds.add(record.get("id").asString());
            });
            ...
            var relatedEntityIds = new ArrayList<String>();
            ...
            relatedResult.forEachRemaining(record -> {
                String id1 = record.get("id1").asString();
                String id2 = record.get("id2").asString();
            ...
            var nodeIdSet = new java.util.HashSet<String>();
            
            nodeResult.forEachRemaining(record -> {
                String nodeId = record.get("id").asString();
     ...
     * @return 路径列表
     */
    public List<Map<String, Object>> findPath(String startEntityId, String endEntityId, int maxDepth) {
        return neo4jDriverFactory.callableRead(tx -> {
            var result = tx.run("""
     ...
     * @return 包含节点和边的图谱数据
     */
    public Map<String, Object> queryGraphByEntityIds(List<String> entityIds, int maxDepth) {
        if (entityIds == null || entityIds.isEmpty()) {
            return Map.of("nodes", List.of(), "edges", List.of());
        }
